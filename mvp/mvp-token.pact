...
(module mvp-token GOV
  "`mvp-token` an example real-world asset (RWA) token, which extends fungible-v2        \
  ...
  (defconst TOKEN-ID:string "RWA.mvp-token" "Token ID")
  ...
  (defun init (name:string symbol:string decimals:integer kadenaID:string compliance:[module{RWA.compliance-v1}] paused:bool owner-guard:guard)
    ...
    (with-capability (GOV)
      ...
      (let ((default-compliance-parameters:object{compliance-parameters-input} 
             { "max-balance-per-investor": -1.0
              ,"supply-limit": -1.0
              ,"max-investors": -1 })
            (compliance:[module{RWA.compliance-v1}] (sort-compliance compliance)) )
  ...
  (defun enforce-unique-compliance:bool (compliance:[module{RWA.compliance-v1}])
  ...
  (defun set-compliance:string (compliance:[module{RWA.compliance-v1}])
    ...
    (only-agent OWNER)
    (let ((new-compliance:[module{RWA.compliance-v1}] (sort-compliance compliance)))
  ...
  (defun sort-compliance:[module{RWA.compliance-v1}] (compliance:[module{RWA.compliance-v1}])
    @doc "Sort compliance contracts by their module name"
    (let ( 
            ;; Create compliance object with module name and reference
            (compliance-object
              (map (lambda (cmpl:module{RWA.compliance-v1})
                { "name": (show cmpl)
                 ,"ref": cmpl
                }) compliance))
            ...
            (sorted-refs:[module{RWA.compliance-v1}] (map (lambda (cmpl-obj) (at "ref" cmpl-obj)) sorted))
          )
      sorted-refs)
    )
